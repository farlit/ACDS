...
class BertSegPos(nn.Module):
    def __init__(self, config, gram2id):
        ...
        self.num_segposlabels = label.num_segposlabels
        ...
        if config.use_augment:
            self.transform2noise_seg = nn.Linear(self.num_seglabels, self.num_seglabels, bias=False)
            self.transform2noise_pos = nn.Linear(self.num_poslabels, self.num_poslabels, bias=False)
            # self.softmax = nn.Softmax(dim=1)
            torch.nn.init.eye_(self.transform2noise_seg.weight.data)
            torch.nn.init.eye_(self.transform2noise_pos.weight.data)
            # # self.tanh = nn.Tanh()
            # self.flag_noise = True
            # transform2noise_seg = torch.empty(size=(self.num_seglabels,self.num_seglabels), requires_grad=True)
            # transform2noise_pos = torch.empty(size=(self.num_poslabels,self.num_poslabels), requires_grad=True)
            # self.transform2noise_seg = nn.Parameter(transform2noise_seg)
            # self.transform2noise_pos = nn.Parameter(transform2noise_pos)
            # self.register_parameter('transform2noise_seg', self.transform2noise_seg)
            # self.register_parameter('transform2noise_pos', self.transform2noise_pos)
            # self.softmax = nn.Softmax(dim=0)
        else:
            self.transform2noise_seg = None
            self.transform2noise_pos = None
        self.crf_seg = CRF(self.num_seglabels, batch_first=True)
        self.crf_pos = CRF(self.num_poslabels, batch_first=True)
        # self.crf_segpos = CRF(self.num_segposlabels, batch_first=True)
        # self.init_weights()
    ...
    def forward(self, input_data, token_type_ids=None, attention_mask=None, seglabels=None, 
                ...
                reduction='sum', mode='train', augment=False):
        ...
        # if self.transform2noise_seg and mode=='train' and augment:
        #     logits_seg = self.transform2noise_seg(logits_seg)
        #     logits_pos = self.transform2noise_pos(logits_pos)

            # logits_seg = self.tanh(logits_seg_temp)
            # logits_pos = self.tanh(logits_pos_temp)
            # seg_noise_matrix = self.softmax(self.transform2noise_seg)
            # pos_noise_matrix = self.softmax(self.transform2noise_pos)
            # print(seg_noise_matrix)
            # logits_seg = torch.matmul(logits_seg,seg_noise_matrix)
            # logits_pos = torch.matmul(logits_pos,pos_noise_matrix)
        ...
        if segposlabels is not None:
            ...
            if lossseg_mask is not None:
                ...
                seg_loss = loss_seg(logits_seg, seglabels, activeseg_mask, reduction) * (-1)
            else:
                seg_loss = loss_seg(logits_seg, seglabels, reduction=reduction) * (-1)

            if losspos_mask is not None:
                activepos_mask = losspos_mask == 1
                pos_loss = loss_pos(logits_pos, poslabels, activepos_mask, reduction) * (-1)
            else:
                pos_loss = loss_pos(logits_pos, poslabels, reduction=reduction) * (-1)
            ...
            # else:
            #     segpos_loss = loss_segpos(logits_segpos, segposlabels) * (-1)
            if reduction == 'sum':
                outputs = (seg_loss+pos_loss,) + outputs
            else:
                outputs = (seg_loss, pos_loss,) + outputs
            # if reduction == 'sum':
            #     outputs = (segpos_loss,) + outputs
            # else:
            #     outputs = (segpos_loss,) + outputs
        return outputs
