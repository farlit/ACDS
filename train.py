...
val_sentences,val_seg,val_pos,val_segpos,flag,val_gram_list,val_positions,val_gram_maxlen,gram2id=load_data('./zuozhuan_train_utf8.txt')
...
if args.local_rank == 0:
    if config.load_before:
        ...
        pre_model = torch.load('model_aug_trans0.pth')
...
for epoch in range(1, config.epoch_num + 1):
    ...
    if args.local_rank == 0:
        ...
        if improve_f1 > 1e-5:
            ...
            #  选择一个进程保存
        # if args.local_rank == 0:
            torch.save(model.module.state_dict(), 'model_aug_trans1.pth')
