...
if args.local_rank == 0:
    if config.load_before:
        # model.load_state_dict(torch.load('model_retry.pth'))
        pre_model = torch.load('model_aug_clean1.pth')
        ...
        print('load_before_model success!')
    torch.distributed.barrier()   # Make sure only the first process in distributed training will download model & vocab
...
for epoch in range(1, config.epoch_num + 1):
    ...
    if args.local_rank == 0:
        ...
        if improve_f1 > 1e-5:
            ...
            #  选择一个进程保存
        # if args.local_rank == 0:
            torch.save(model.module.state_dict(), 'model_aug_alldata1.pth')
